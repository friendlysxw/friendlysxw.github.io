...
# EOF
  ...
  git init
  git config user.name "friendlysxw"
  git config user.email "friendlysxw@163.com"
  git add .
  git commit -m "Update Blog By TravisCI With Build $TRAVIS_BUILD_NUMBER"
  # Github Pages
  git push --force --quiet "https://${github_blog_token_name}@${GH_REF}" master:master

  # Create Tag
  git tag v1.2.$TRAVIS_BUILD_NUMBER -a -m "Auto Taged By TravisCI With Build $TRAVIS_BUILD_NUMBER"
  # Github Pages
  git push --quiet "https://${github_blog_token_name}@${GH_REF}" master:master --tags
}
